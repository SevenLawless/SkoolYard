...
export default function EditUserPage() {
  ...
  if (!user) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ href: "/dashboard", label: "Dashboard" }, { href: "/admin/users", label: "User Management" }, { label: "Edit" }]} />
  ...
  const validateForm = () => {
    ...
    const existingUser = data.users.find((u) => u.username === formData.username && u.id !== user.id);
    if (existingUser) {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <div className="flex items-center gap-4 mt-4">
          ...
          <h1 className="section-title text-gradient">Edit {user.name}</h1>
        </div>
        <p className="muted text-sm">Update user account information</p>
      </div>
