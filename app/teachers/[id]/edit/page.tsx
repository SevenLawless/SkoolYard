...
export default function EditTeacherPage() {
  const { data, updateTeacher } = useData();
  ...
  useEffect(() => {
    if (teacher) {
      reset({
        ...
        hasDiscount: teacher.hasDiscount,
        discountPercentage: teacher.discountPercentage,
      });
    }
  }, [teacher, reset]);
  ...
  if (!teacher) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ href: "/dashboard", label: "Dashboard" }, { href: "/teachers", label: "Teachers" }, { label: "Edit" }]} />
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <Breadcrumbs items={[
          ...
          { href: `/teachers/${id}`, label: teacher.name },
          { label: "Edit" }
        ]} />
        <div className="flex items-center gap-4 mt-4">
          <BackButton href={`/teachers/${id}`} />
          <h1 className="section-title text-gradient">Edit {teacher.name}</h1>
        </div>
        <p className="muted text-sm">Update teacher information</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Input label="Subjects (comma separated)" {...register("subjects")} />
